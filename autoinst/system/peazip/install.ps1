if ((New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent())).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
	...
	$SetupFile = ""
	$Uri = ""
	$LatestRelease = Get-LatestRelease -Owner "peazip" -Project "PeaZip"
	$LatestRelease.assets | ForEach-Object {
		if($_.name -match "^peazip-.*\.WIN64.exe$"){
			$SetupFile += $_.name
	...
	Start-Process -Wait -FilePath "${TempDirectory}\${SetupFile}" -ArgumentList @("/SILENT", "/MERGETASKS=!desktopicon")
