if ((New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent())).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
	...
	function Get-Version {
		...
		$FilesPage = Invoke-WebRequest -Uri "https://download.virtualbox.org/virtualbox/${LatestVersion}/"
		$VersionData = $null
		if ("$FilesPage" -match '<a href="Oracle_VirtualBox_Extension_Pack-(.*)\.vbox-extpack">.*<\/a>') {
			$VD = $Matches.1 -split "-"
			$VersionData = @{
				Version  = $VD[0];
				Revision = $VD[1]
	...
	$vboxversrev = "$($VersionData.Version)-$($VersionData.Revision)"
