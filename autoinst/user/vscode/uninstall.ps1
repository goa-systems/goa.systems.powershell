...
function Get-UninstString {
	...
	$var1 = Get-ChildItem -Path $regkey | Get-ItemProperty | Where-Object { $_.DisplayName -match "Microsoft Visual Studio Code" }
...
foreach($regkey in $regkeys){
	...
	if([string]::IsNullOrEmpty($uninststring)){
		...
		Write-Host -Object "Uninststring found $uninststring. Running with parameter /SILENT."
		Start-Process "$uninststring" -ArgumentList "/SILENT" -Wait
	}
}