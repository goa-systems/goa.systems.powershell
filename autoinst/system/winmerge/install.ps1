if ((New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent())).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
	...
	$DownloadUrl="https://github.com/WinMerge/winmerge/releases/download/$version/$setup"
	...
	if ( -Not (Test-Path -Path "$env:SystemDrive\ProgramData\InstSys\$name\$setup")) {
		Start-BitsTransfer -Source "$DownloadUrl" -Destination "$env:SystemDrive\ProgramData\InstSys\$name\$setup"
	}
